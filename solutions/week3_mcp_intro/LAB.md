...
## 1. Objectives
By the end you can:
Wrap a deterministic REST (or in-process) weather provider as a tool.

Register & expose a descriptor (name, description, inputs, outputs).

Parse natural language → intent + parameters.

Invoke tool only when needed & cite response.

Log invocation metrics (latency, success, tool_used)
